...
def binary_op_type_checking_impl(
    ...
    if arithmetic_check and not lhs_sca_ty.is_ptr() and not rhs_sca_ty.is_ptr():
        ret_sca_ty = computation_type_impl(
        ...
        lhs = (
            full((), lhs_scalar, dtype=ret_sca_ty, builder=builder)
            if lhs_is_scalar
            else cast(lhs, ret_sca_ty, builder)
        )
        rhs = (
            full((), rhs_scalar, dtype=ret_sca_ty, builder=builder)
            if rhs_is_scalar
            else cast(rhs, ret_sca_ty, builder)
        )
...
def binary_op_sanitize_overflow_impl(
    lhs: tl.tensor, rhs: tl.tensor, builder: ir.builder, binary_op: callable
):
...
def floordiv(
    ...
    if input_scalar_ty.is_int() and other_scalar_ty.is_int():
        ...
        if ret_ty.is_int_signed():
            return tl.tensor(
                builder.create_sdiv(input.handle, other.handle), input.type
            )
        else:
            return tl.tensor(
                builder.create_udiv(input.handle, other.handle), input.type
            )
    raise TypeError(f"unexpected type {input_scalar_ty}")
...
def mod(
    ...
    # % int
        ...
        if scalar_ty.is_int_signed():
            return tl.tensor(
                builder.create_srem(input.handle, other.handle), input.type
            )
        else:
            return tl.tensor(
                builder.create_urem(input.handle, other.handle), input.type
            )
    raise TypeError(f"unexpected type {scalar_ty}")
...
def minimum(
    x: tl.tensor, y: tl.tensor, propagate_nan: tl.PropagateNan, builder: ir.builder
):
...
def invert(input: tl.tensor, builder: tl.tensor) -> tl.tensor:
    ...
    _1 = tl.tensor(
        builder.get_all_ones_value(input_sca_ty.to_ir(builder)), input_sca_ty
    )
...
def reshape(
    input: tl.tensor, dst_shape: List[int], can_reorder: bool, builder: ir.builder
) -> tl.tensor:
    numel = 1
    for s in dst_shape:
        numel *= s
    ...
    return tl.tensor(
        builder.create_reshape(input.handle, dst_shape, can_reorder), ret_ty
    )
...
def cat(
    lhs: tl.tensor, rhs: tl.tensor, can_reorder: bool, builder: ir.builder
) -> tl.tensor:
...
def broadcast_impl_shape(
    input: tl.tensor, shape: List[int], builder: ir.builder
) -> tl.tensor:
    if not input.type.is_block():
...
def broadcast_impl_value(
    lhs: tl.tensor, rhs: tl.tensor, builder: ir.builder
) -> tl.tensor:
    lhs_ty = lhs.type
    rhs_ty = rhs.type
...
def cast(
    ...
    if src_sca_ty.is_fp8e4b15() or dst_sca_ty.is_fp8e4b15():
        ...
        return builder.codegen_fns["convert_custom_types"](
            input, dst_ty, fp_downcast_rounding, _builder=builder
        )
    ...
    if (
        ...
        return tl.tensor(
            builder.create_fp_to_fp(
                input.handle, dst_ty.to_ir(builder), fp_downcast_rounding
            ),
            dst_ty,
        )
        ...
        if dst_sca_ty.is_bool():
            ...
            return tl.tensor(
                builder.create_int_cast(
                    input.handle, dst_ty.to_ir(builder), sign_extend
                ),
                dst_ty,
            )
    ...
    if src_sca_ty.is_standard_floating() and dst_sca_ty.is_int():
        if dst_sca_ty.is_bool():
            ...
            return tl.tensor(
                builder.create_fp_to_ui(input.handle, dst_ty.to_ir(builder)), dst_ty
            )
...
def _load_block_pointer(
    ...
    # Build IR
    return tl.tensor(
        builder.create_tensor_pointer_load(
...
def descriptor_store(
    desc_ptr: tl.tensor, value: tl.tensor, offsets, builder: ir.builder
) -> tl.tensor:
    offsets = _convert_to_ir_values(builder, offsets, require_i64=False)
    return tl.tensor(
        builder.create_descriptor_store(desc_ptr.handle, value.handle, offsets), tl.void
    )
...
def tensormap_fenceproxy_acquire(desc_ptr: tl.tensor, builder: ir.builder) -> tl.tensor:
    return tl.tensor(
        builder.create_tensormap_fenceproxy_acquire(desc_ptr.handle), tl.void
    )
...
def _store_block_pointer(ptr, val, mask, boundary_check, cache, eviction, builder):
    ...
    # Build IR
    return tl.tensor(
        builder.create_tensor_pointer_store(
...
def _store_legacy(ptr, val, mask, boundary_check, cache, eviction, builder):
    ...
    if not mask:
        return tl.tensor(
            builder.create_store(ptr.handle, val.handle, cache, eviction), tl.void
        )
...
def store(
    ...
    if ptr.type.is_ptr() and ptr.type.element_ty.is_block():
        # Store by a block pointer: `pointer_type<block_type<>>`
        return _store_block_pointer(
            ptr, val, mask, boundary_check, cache, eviction, builder
        )
    else:
        # Store by a tensor of pointers or a pointer of scalar: `block_type<pointer_type<>>` or `pointer_type<>`
        ...
        if sca_ty.is_int_signed():
            return tl.tensor(
                builder.create_atomic_rmw(
                    ir.ATOMIC_OP.MAX, ptr.handle, val.handle, mask.handle, sem, scope
                ),
                val.type,
            )
        else:
            return tl.tensor(
                builder.create_atomic_rmw(
                    ir.ATOMIC_OP.UMAX, ptr.handle, val.handle, mask.handle, sem, scope
                ),
                val.type,
            )
    # for float
...
def atomic_min(
    ...
    if sca_ty.is_int():
        if sca_ty.is_int_signed():
            return tl.tensor(
                builder.create_atomic_rmw(
                    ir.ATOMIC_OP.MIN, ptr.handle, val.handle, mask.handle, sem, scope
                ),
                val.type,
            )
        else:
            return tl.tensor(
                builder.create_atomic_rmw(
                    ir.ATOMIC_OP.UMIN, ptr.handle, val.handle, mask.handle, sem, scope
                ),
                val.type,
            )
    # for float
...
def dot(
    ...
    assert (
        builder.codegen_fns.get("min_dot_size") is not None
    ...
    return tl.tensor(
        builder.create_dot(
            lhs.handle, rhs.handle, acc_handle, input_precision, max_num_imprecise_acc
        ),
        ret_ty,
    )
...
def where(
    condition: tl.tensor, x: tl.tensor, y: tl.tensor, builder: ir.builder
) -> tl.tensor:
    ...
    ret_ty = x.type
    return tl.tensor(
        builder.create_select(condition.handle, x.handle, y.handle), ret_ty
    )
...
def reduction(
    ...
    if axis is None:
        inputs = tuple(
            reshape(t, [t.numel.value], can_reorder=True, builder=builder)
            for t in inputs
        )
        axis = 0
    # get result shape
    ...
    assert all(
        t.type.shape == shape for t in inputs
    ...
    return tuple(
        wrap_tensor(reduce_op.get_result(i), inputs[i].type.scalar, ret_shape)
...
def associative_scan(
    ...
    return tuple(
        wrap_tensor(scan_op.get_result(i), inputs[i].type.scalar, shape)
        for i in range(len(inputs))
    )
...
def device_print(
    prefix: str, args: List[tl.tensor], hex: bool, builder: ir.builder
) -> tl.tensor:
    ...
    is_signed = [
        arg.dtype in (tl.int1, tl.int8, tl.int16, tl.int32, tl.int64) for arg in args
    ]
...
def _convert_elem_to_ir_value(builder, elem, require_i64):
    ...
    if isinstance(elem, tl.constexpr):
        ...
        if elem.dtype != tl.int64 and require_i64:
            return builder.create_int_cast(
                elem.handle, builder.get_int64_ty(), elem.dtype.is_int_signed()
            )
...
def _convert_to_ir_values(builder, list_like, require_i64=True):
    if hasattr(list_like, "__iter__"):
        return [
            _convert_elem_to_ir_value(builder, elem, require_i64) for elem in list_like
        ]
    return [_convert_elem_to_ir_value(builder, list_like, require_i64)]


def make_block_ptr(
    base: tl.tensor, shape, strides, offsets, block_shape, order, builder: ir.builder
) -> tl.tensor:
    ...
    block_shape = [
        elem.value if isinstance(elem, tl.constexpr) else elem for elem in block_shape
    ]
    assert all(
    ...
    handle = builder.create_make_block_ptr(
